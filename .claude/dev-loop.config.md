...
Python safe-test totals are generated by `make test` with ML disabled and a
3GB cap; the last recorded safe run had 13,910 passing and 29 skipped.
TypeScript inventory: 40 test files under `tests/ts/`; run
`bun test tests/ts/` for the current assertion count.
...
notes:
  ...
  python_runtime: scripts/python_runtime.sh via Makefile `PYTHON_RUNTIME`
    (prefers `uv run python`, falls back to `.venv/bin/python`, and fails
    closed instead of using an arbitrary global interpreter)
  ...
  gotchas: |
    ...
    - `public/data/prices.json` is generated by `bun run fetch-data`. At this
      revision, the checked-in snapshot covers 38 symbols with 5,404 SPY rows
      from 2005-01-03 to 2026-06-26. Recompute with the `CONTEXT.md` command
      before citing a live count.
