...
def main():
    ...
    logger.info("=" * 60)
    logger.info("BAYESIAN VOLATILITY — %s", symbol)
    logger.info("=" * 60)
    logger.info("Timestamp: %s", result.timestamp)
    logger.info("Observations: %d", result.n_obs)
    logger.info("")
    logger.info("%-25s %12s %12s", "Component", "Vol", "Precision")
    logger.info("-" * 50)
    logger.info("%-25s %11.2f%% %11.1f", "Prior (long-term)", result.prior_vol * 100, result.prior_precision)
    logger.info("%-25s %11.2f%% %11.1f", "Likelihood (recent)", result.likelihood_vol * 100, result.likelihood_precision)
    logger.info("%s", "─" * 50)
    logger.info("%-25s %11.2f%%", "Posterior (Bayesian)", result.posterior_vol * 100)
    logger.info("")
    logger.info("95%% Credible Interval: [%.2f%%, %.2f%%]", result.credible_interval_lower * 100, result.credible_interval_upper * 100)
    logger.info("Shrinkage toward prior: %.1f%%", result.shrinkage_factor * 100)
    logger.info("Regime scale: %.2fx", result.regime_scale)
    logger.info("High vol regime: %s", result.is_high_vol_regime)
    logger.info("")
    logger.info("Naive mean: %.2f%%", result.simple_mean_vol * 100)
    logger.info("Bayesian adjustment: %+.2f%%", (result.posterior_vol - result.simple_mean_vol) * 100)
    logger.info("=" * 60)
