...
Beat 217: multi-fixture **idle** Session A ``--candidate-json`` **directory** (exists but is not a file) fail live vs ``--candidate-json --dry-run`` fail (SystemExit, not verdict=failed) via tmp_path (promotes Beat 32 unit → multi-fixture idle contrast; starts next I/O leftover after boolean trilogy 214–216 closed; same idle fixtures as Beat 171/175/185/187/195/198/202/205/208/211/214; mkdir cand-dir each test; one representative path only — keep exactly 2 tests; NEVER mutate fixtures): for each fixture session-a ``--candidate-json`` directory ``--json`` (NO dry-run)→SystemExit nonzero ``--candidate-json`` + ``not a file`` plan UNCHANGED no success JSON; session-a ``--candidate-json`` directory ``--dry-run --json``→same SystemExit plan UNCHANGED (dry-run does not skip directory load); Watch fixtures keep ``## Watch`` + ``## Heartbeat``; ``watch_heartbeat_no_queue`` still has no ``## Queue``. Contrast Beat 198 (missing-file SystemExit idle) + Beat 32 (directory unit); Beat 217 is directory candidate-json CLI load-fail live+dry-run on idle fixtures. Proof: pytest ``-k beat217``.
Beat 218: multi-fixture **non-pickable** Session A ``--candidate-json`` **directory** (exists but is not a file) fail live vs ``--candidate-json --dry-run`` fail (SystemExit, not verdict=failed) via tmp_path (parallel Beat 217 idle directory SystemExit + Beat 199 nonpick missing-file SystemExit; continues directory trilogy after Beat 32 unit → Beat 217 idle; fixtures ``shipped_only`` / ``incomplete_open`` / ``broken_ready_flag`` / ``open_complete_not_ready`` / ``contract_spec`` / ``queue_with_watch_heartbeat``; mkdir cand-dir each test; NEVER mutate fixtures): for each fixture session-a ``--candidate-json`` directory ``--json`` (NO dry-run)→SystemExit nonzero ``--candidate-json`` + ``not a file`` plan UNCHANGED no success JSON; session-a ``--candidate-json`` directory ``--dry-run --json``→same SystemExit plan UNCHANGED (dry-run does not skip directory load); ``queue_with_watch_heartbeat`` keeps ``## Watch`` + ``## Heartbeat`` with Watch before Queue. Contrast Beat 217 (directory SystemExit idle) + Beat 199 (missing-file SystemExit nonpick) + Beat 32 (directory unit); Beat 218 is directory candidate-json CLI load-fail live+dry-run on non-pickables. Proof: pytest ``-k beat218``.
