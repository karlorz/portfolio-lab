...
def main():
    ...
    logger.info("=" * 60)
    logger.info("CASHLESS COLLAR SIGNAL GENERATOR v4.60")
    logger.info("=" * 60)
    logger.info("Timestamp: %s", signal.timestamp)
    logger.info("State: %s", signal.signal_state)
    logger.info("Regime: %s", signal.regime)
    logger.info("")
    logger.info("SPY Price: $%.2f", signal.underlying_price)
    logger.info("VIX: %.1f", signal.vix_level)
    logger.info("")
    logger.info("Collar Strikes:")
    logger.info("  Call (short): $%.2f (+%.1f%% cap)", signal.call_strike, signal.max_upside_pct)
    logger.info("  Put  (long):  $%.2f (-%.1f%% floor)", signal.put_strike, signal.max_downside_pct)
    logger.info("")
    logger.info("Net Premium: $%.2f per share", signal.strikes.net_premium)
    logger.info("Annualized Yield: %.2f%%", signal.expected_monthly_yield)
    logger.info("Cashless: %s", signal.strikes.is_cashless)
    logger.info("")
    logger.info("Notional: %.0f%% of portfolio", signal.collar_notional_pct * 100)
