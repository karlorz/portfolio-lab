...
def main():
    ...
    logger.info("=" * 60)
    logger.info("CRYPTO TACTICAL ALLOCATION SIGNAL v4.70")
    logger.info("=" * 60)
    logger.info("Timestamp: %s", signal.timestamp)
    logger.info("State: %s", signal.signal_state)
    logger.info("Valid: %s", signal.is_valid)
    logger.info("")

    logger.info("BTC Signal:")
    logger.info("  Price: $%.0f", signal.btc_signal.price)
    logger.info("  6m Momentum: %.1f%%", signal.btc_signal.momentum_6m * 100)
    logger.info("  3m Momentum: %.1f%%", signal.btc_signal.momentum_3m * 100)
    logger.info("  30d Vol: %.1f%%", signal.btc_signal.vol_30d * 100)
    logger.info("  Regime: %s", signal.btc_signal.vol_regime)
    logger.info("  State: %s", signal.btc_signal.signal_state)
    logger.info("")

    logger.info("ETH Signal:")
    logger.info("  Price: $%.0f", signal.eth_signal.price)
    logger.info("  6m Momentum: %.1f%%", signal.eth_signal.momentum_6m * 100)
    logger.info("  3m Momentum: %.1f%%", signal.eth_signal.momentum_3m * 100)
    logger.info("  30d Vol: %.1f%%", signal.eth_signal.vol_30d * 100)
    logger.info("  Regime: %s", signal.eth_signal.vol_regime)
    logger.info("  State: %s", signal.eth_signal.signal_state)
    logger.info("")

    logger.info("Composite:")
    logger.info("  Crypto Weight: %.2f%%", signal.composite_weight * 100)
    logger.info("  GLD Reduction: %.2f%%", signal.gld_reduction * 100)
    logger.info("  Vol Scale: %.2fx", signal.vol_scale_factor)
