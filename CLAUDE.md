...
|- **CI cron-sync gate**: GitHub Actions python job now runs `CI=true make verify-cron-sync`; Makefile CI mode synthesizes cron_status via cron-reset, verifies crontab target coverage + tasker registry integrity, and skips host-local Hermes/wiki checks
