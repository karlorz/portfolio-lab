...
|- **Cron Python runtime launcher**: scripts/python_runtime.sh routes scheduled Python through `uv run python` or `.venv/bin/python` instead of bare `python3`; Makefile cron targets plus repo/Hermes wrappers use PYTHON_RUNTIME; smoke covers numpy/cachetools/scipy/pypfopt
