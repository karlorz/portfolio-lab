...
|- **Decision replay + experiment registry**: `src/monitor/decision_registry.py` — SQLite ledger (`data/decision_registry.db`) records dashboard-cycle ensemble/rebalance decisions, **evaluator** paper rebalance/kill-switch cycles, **backtest** rows via `save_results_json(..., experiment_manifest=...)`, syncs offline `labs_registry.json` experiment rows, publishes `public/data/decision_registry.json` with replay summaries + promotion gate; React **Decisions** tab (`DecisionReplayPanel`, Zod `schemas/decision_registry.ts`); 13 tests (`test_decision_registry.py`, `test_decision_registry_delivery.py`)
