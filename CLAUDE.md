...
| - Test count: **12425 safe** (12213 Python + 214 TypeScript, 27 skipped, 0 failures, 42 BL mapper tests gated behind pypfopt)
...
|- **TTL price cache**: src/data/price_cache.py — cachetools.TTLCache(maxsize=1, ttl=30s) eliminates redundant prices.json reads across 5 signal modules (PRICE_CACHE_TTL_SECONDS env var), ~10MB peak memory savings per cron cycle
