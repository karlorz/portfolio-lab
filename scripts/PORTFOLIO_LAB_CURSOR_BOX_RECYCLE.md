...
> authority change. Every action it describes is a read-only probe except the
> operator action placeholder, which requires separate attended confirmation
> at execution time. This document is not evidence that any recycle occurred;
> evidence of record is the attended recycle proof (`recycle.json`) accepted
> by the seven-day acceptance checker.
...
| `archives/` | local timestamped archive tarballs + `.sha256` sidecars |
...
Read-only probes only. `status --read-only` never cleans stale PID/state
records and never mutates services.

1. **Fresh same-day successful archive + SHA.** `run/s3-archive-last-utc-day`
   equals today's `%Y%m%d`; `run/s3-archive.log` latest success line is
   `published ... sha256=<64-hex>` on today's UTC day; the local tarball
   `archives/portfolio-lab-<STAMP>.tar` and its `.sha256` sidecar exist
   (mode 0600); the S3 object key
   `daily/YYYY/MM/DD/portfolio-lab-data-<utc>.tar` + `.sha256` published.
   Verify read-only:
   `python3 app/scripts/portfolio_lab_recovery.py verify --archive /home/box/.local/share/portfolio-lab/archives/portfolio-lab-<STAMP>.tar`
   (sidecar is derived at `<archive>.sha256`).
2. **Current daily evidence pass, not warning.** Today's UTC day evidence
   ...
   `ready_except_recycle` when recycle.json is absent:
...
3. **Fresh mode-0600 sg01 proof, all false.** `run/former-authority-proof.json`
   (schema `portfolio-lab-former-authority-proof/v1`): regular non-symlink,
   exactly mode 0600, `host_label` `sg01`, `collected_at` fresh under the
   freshness max age (default 21600 s) and not future-dated, and
   `tasker.active`, `tasker.enabled`, `archive_timer.active`,
   `archive_timer.enabled` all exactly `false`. Refresh it attended before
   the recycle; never assume an aged proof.
4. **Exact one scheduler.** Tasker controller reports `scheduler_instances: 1`:
...
5. **Tasker active/exact** (same probe): `state: "active"` and
   `identity_exact: true`.
6. **Static active/exact**:
   ...
   `identity_exact: true`).
7. **Exactly one tunnel connection.** Attended operator check (no shipped
   controller covers the tunnel): exactly one active tunnel connection/process
   serving `lab.termolo.com` to the loopback origins. The operator's existing
   tunnel verification applies; record the judgment `pass`/`fail` attended.
   A count other than one blocks.
8. **API and static HTTP 200.** `http://127.0.0.1:8000/api/tasker/status`
   and `http://127.0.0.1:8001/` both return HTTP 200 (loopback probes;
   bounded, e.g. 10 s default like the collector).
9. **Disk >= 15 GiB free.** `resources` evidence `disk.status: "pass"` with
   `free_bytes >= 15 GiB` (15 * 1024^3); below that the collector warns and
   the acceptance criterion fails.
10. **Backups and rollback commands identified.** The timestamped deployment
    backup is the verified archive from precondition 1 (local tarball, its
    sidecar, and the S3 object). Pre-identified rollback (not run from this
    runbook; requires separate approval per Failure path):
    `portfolio_lab_recovery.py restore --archive <verified .tar> --app-dir <app> --web-root <www> --target-mode prod --allow-production-paths --service-controller box-persist`,
...
- Validate read-only before finalizing:
  ...
  must produce verdict `accept` (phases fill only after the post-recycle
  checks; any earlier command run with the file absent yields
  `ready_except_recycle`, which is fine and expected).
...
6. The operator has the verified timestamped archive and the rollback
   command identities at hand, and another human or recorded approval exists
   per the attended approval gate.
...
Failure path. All probes read-only.

1. **Host reachable.** cursor-box responds to an attended reachability probe
   (e.g. SSH/ICMP, bounded).
2. **box-persist ensure has run.** Production Tasker and static are only
   restarted from inactive by the ensure hook (production `ensure` requires
   the exact `run/production-activation.json` marker match): both controllers
   reporting `active` with `identity_exact: true` is the observable proof
   that ensure ran; the activation marker must still match exactly.
3. **Tasker active/exact, exactly one scheduler** (precondition 4/5 probes).
4. **Static active/exact** (precondition 6 probe).
5. **Exactly one tunnel connection** (precondition 7 attended check).
6. **API and static HTTP 200** (precondition 8 probes).
7. **Evidence collection rerunnable/read-only.** Re-run today's collector
   with the same UTC day pinned (`--now` on that day): same-day reruns
   replace files atomically and observation-derived files are byte-identical
   under the same inputs; the collector never starts/stops services. The
   rerun must complete and today's evidence must again be overall `pass`.
8. **sg01 remains disabled via fresh attended proof.** Refresh
   `run/former-authority-proof.json` attended (fresh `collected_at`, all four
   booleans false, mode 0600) and re-run the collector so the current day's
   `authority` category is `pass` with `host_label: "sg01"`.
...
- **Rollback uses the timestamped deployment backup** (the verified archive
  and sidecar from precondition 1, and/or its S3 object), never a live state
  merge.
- **Rollback always stops cursor-box Tasker first** (box-persist controller
  ...
  separate attended approval — the pre-identified command in precondition 10
...
| `portfolio-lab-former-authority-proof/v1` | daily evidence `collect_authority` |
| Archive + SHA sidecar | `scripts/cron/portfolio-lab-cursor-box-s3-archive.sh`, `scripts/portfolio_lab_s3_archive.py`, recovery `verify` |
| Restore/rollback | `scripts/portfolio_lab_recovery.py` (`restore --target-mode prod --service-controller box-persist`) |