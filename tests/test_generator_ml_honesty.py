#!/usr/bin/env python3
"""
Generator ML-honesty tests — ML signal disclosure and export-contract classes
(TEST-GENERATOR-SPLIT s8, 2026-08-12).

Moved verbatim from tests/test_generator.py (9 ML-honesty classes per the
15:20Z table; note TestMlSignalsEdgeCases lowercase-l vs
TestMLSignalsEdgeCasesExtended uppercase — distinct classes) — no tests
renamed or weakened. Shared helpers live in tests/helpers.py (plain module;
the autouse fixture below is duplicated verbatim per split file — never move
it to conftest.py, it would pollute the full ~15k-test suite).
"""
import json
import sys
import types
